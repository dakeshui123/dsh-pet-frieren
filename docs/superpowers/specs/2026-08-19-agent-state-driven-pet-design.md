...
| 拖拽 | 移动桌宠（位置记忆保持不变）。拖拽中水平位移 dx<0 → row 2 向左跑；dx>0 → row 1 向右跑；纯垂直拖动保持当前动画。松手后恢复常驻动画 |
| 悬停 | 桌宠右上角浮现小 ✕ 关闭按钮；点击隐藏（持久化到 localStorage，键沿用现有 HIDDEN_KEY） |
| 隐藏后恢复 | 原位置显示一枚半透明小徽章（约 36px，桌宠头像图样），悬停提示"显示桌宠"，点击恢复显示 |
| 空闲自动 | 仅 idle 期间：每 12~30s 自动挥手一次；每 45~90s 偶尔自动跳跃一次。Agent 忙碌时不自动触发 |
| 主动点击 | 任何 Agent 状态下均响应（作为临时覆盖层动画，结束后恢复常驻动画） |
| reduce-motion | 桌宠静止：显示当前 Agent 状态的静态帧（随状态切换更新）；自动动画与点击动画全部禁用 |
...
| `README.md` | 更新 Interactions 表、"How it works" 段（注入 sessions 服务）、中文段 |
