...
| 操作 | 效果 |
| --- | --- |
| （无操作） | 跟随 Agent 状态：空闲时发呆循环；运行时小跑；等待审批/提问时等待；计划评审时审阅；Agent 报错时扑倒（定格最后一帧直到错误清除）。切换会话时桌宠随之切换。 |
| 等待（仅空闲时） | 每 12–30 秒自动挥手一次，每 45–90 秒偶尔跳一下 |
| 单击 | 挥手一次 |
| 双击 | 跳跃一次 |
| 拖拽 | 移动桌宠（位置会被记住）；左右拖拽时朝对应方向跑 |
| 悬停 | 角落浮现小 ✕，点击隐藏桌宠 |
| 隐藏后 | 原地留下一枚半透明小徽章，点击恢复 |
...
`pendingInteraction`）以及当前会话的实时快照（`lastAgentError`）。映射：
